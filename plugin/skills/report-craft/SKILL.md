...
The recommendation is first, not the setup, not the method, not the history of how the finding was reached - the founder's standing order for every deliverable. "Answer first" above is that law applied to reports: paragraph one carries the call, and the Evidence and Appendix sections that follow are justification for a conclusion the reader already has, never a trail the reader must walk to find it. When the report enumerates items - three renewal risks, five findings, a Tier A/B/C list - the same law runs at the item level: each item's own recommendation, disposition, or verdict renders in its own visually distinct block (a `.callout` for the one page-level call, a `.verdict` strip for each item's, per `company/design/design-system.md`), never buried mid-paragraph where a reader has to mine for it. Prose still carries the reasoning; the block carries the call. A report that gets this backwards - conclusion at the bottom, evidence up top - reads as a mystery novel, and busy readers do not have time for the reveal.
