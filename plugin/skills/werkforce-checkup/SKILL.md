...
- The script report: OK and WARN lines from `scripts/checkup.sh` covering the OS spine, the whole HQ tree (including the audit log, the onboarding checklist, and the design system), manifest drift, format-comment drift, sessions hygiene, timestamps that look like UTC/Zulu, the filing law's one-home rule, the org chart's twelve rows, seat cards missing any of their six H2s, deliverables missing their `.html` render or carrying a stale one (the `.md` edited since its render was last generated), founder-facing deliverable text that reads like it asks the founder to run git, merge a PR, read CI, or edit a file directly, and every board's shape and staleness - including Blocked rows, plus the hook probe - every registered hook script's existence, executable bit, and evidence it has fired since its last change
...
- **Hook health.** Every hook script this HQ can see, read from every surface Claude Code honors: the settings files (`.claude/settings.json` and `settings.local.json` beside the HQ, in the folder above it, and in `~/.claude`), the standalone `.claude/hooks.json` at those same three levels (where the kernel PreToolUse guard is armed and no settings file ever mentions it), and the plugins those settings enable - and the report names every file the probe actually read, so a probe that saw nothing can never be mistaken for an HQ with nothing armed, each of which registers its own hooks in `<installPath>/hooks/hooks.json` where no settings file ever mentions them. The script inside a hook command is found by scanning the command's tokens for the ones that name a real path, so the common wrapper forms are probed rather than skipped - `bash "<script>"`, `node "<script>"`, and a launcher with a payload (`bash hook-exec.sh guard-delete.mjs`, where the launcher and the guard each get their own line). Each script gets three questions: is it on disk, can it run, and is there evidence it has actually fired since it was last changed. A registered hook that cannot run is a dead tripwire, and a dead tripwire looks exactly like a quiet one, because a well-built hook fails open and never says a word - this HQ's two hooks sat non-executable for a full day before anyone noticed (lesson, 2026-07-24). The executable bit is only checked when the command runs the script *itself*: a script handed to `bash` or `node` by a launcher runs mode 644 by design - every plugin ships them that way - so warning about its exec bit would be noise, and the line says so instead. The firing evidence is read honestly and labelled in the line it prints: a hook may declare where its footprints land with a `# checkup-evidence: <path or glob, relative to the HQ>` comment in its own source, and a matching file newer than the script is proof of a successful run; failing that, a dated `[<hook-name>]` line in `records/warnings.md` proves the hook ran, though that run reported trouble. No evidence is never reported as "dead" - it reads `[unknown]`, because a hook that leaves no trace when it passes cannot be told apart from one that never fires, and the line names the fix: fire it once by hand as an install check, the way every hook install should end. Plugin-provided guards are silent by design - they write nothing anywhere when they pass - so instead of a wall of identical warnings they are counted into one `NOTE` naming them, reported for visibility rather than judged. Inline shell commands and paths holding a variable this probe cannot expand print as `NOTE` too - neither OK nor WARN, because the probe honestly cannot judge them. All of it is WARN-only like every other front here: a dead hook is a flag on the play, never a stopped game.

Then it inspects every department folder: charter, playbook, briefs, board, and memory present, plus `seats/` and `outbox/`. It deliberately never looks inside `drafts/` - work in progress is invisible to controls until handoff, and the script honors that. Each seat card in `seats/` is checked for all six H2s the format law requires (Mission, What excellent looks like, How this seat works, Boundaries, Anti-patterns, Escalation) - a card missing any is a warning, because every seat is defined to the bar of an elite hire. Each finished deliverable in `outbox/` is checked for its `.html` render beside the `.md` source - a deliverable ships as the pair - and for currency: if the `.md` has been edited more recently than its `.html` sibling, the render is stale and the line says so - it names the outcome (regenerate the render from this markdown before shipping), never a specific tool, since this script runs in every customer's HQ and cannot assume which rendering skill or tool that HQ uses. The same pass scans each deliverable's text for phrasing that reads like an instruction to run git, merge a PR, read CI, or edit a file directly - founder-facing text should describe the outcome, never hand the founder the plumbing (charter §14); this is a blunt phrase match, not judgment, so it can over-flag, and it only ever warns. Each board gets the full pass: the standard column row present, Done rows without a receipt, tasks sitting In progress past a week, Blocked rows that do not say what blocks them (`blocked by X - recheck Y` belongs in the Receipt cell), and Blocked rows older than a week - a block wants a recheck, not a burial.
...
The script checks shapes; I check whether the story holds together - whether the org chart and the departments on disk agree in meaning, not just in structure. These are the eight things bash cannot judge, and every one of these findings is `[checked]` - I looked at the files just now, never from memory:
...
- **Desk-executed production.** The main coordinating session - the desk - delegates production; it does not produce. I read this week's `records/worklog.md` and `records/audit-log.md` entries and flag any that names a produced deliverable or internal change whose owner stamp resolves to the desk itself rather than a department Worker seat. Coordination is the desk's job and is never flagged - intake, filing, delegating, running decisions, session lines, board bookkeeping, answering the founder. Production is a seat's job - a deliverable file or an internal change that should have carried a board row, a seat, and independent review. The finding reads plainly: "N desk-executed productions this week - should these have been delegated?" It is a WARN like every other - visibility, never a stop. If the ledgers do not stamp an owner I can tell apart from the seats, I say that instead, once, as its own finding.
- **Registry coherence.** The same ghost-receipt check, extended to the artifact mandate (founder law, 2026-07-23): an outbox `.html` past a Manager-review PASS with no matching line in `records/artifact-registry.md`, and registry lines whose path no longer exists on disk. Either is a warning, never a block - the registry is a derived record of what was published, and drift there is the checkup's job to catch, not the operator's to notice on their own.
...
The warnings file is your company's flight recorder and the improvements file is its classroom; the weekly review reads the classroom and proposes any and all changes the record supports, each atomic. Nothing in either file ever stops work.
