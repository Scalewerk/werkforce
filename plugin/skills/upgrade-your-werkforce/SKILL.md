...
- `TREES.md` - the tree-authority registry the checkup's tripwire reads, seeded from `references/seeds/TREES.md` with the live-HQ row and the ambiguity rule; skipped if it already exists. Without it the tripwire silently skips, which is exactly the kind of blind spot a migration must not leave behind.
...
This step ends with a receipt, not a promise: I re-list every file this step names - the founder-data set above, each v3 addition (`records/audit-log.md`, `records/operator-reviews.md`, `records/artifact-registry.md`, `company/design/`, `company/onboarding.md`, `TREES.md`) - and confirm each now exists on disk, printing one line per file. A file still missing at that point is a failed migration step I report to you by name, never something I skip past silently - the first external upgrade (2026-07-27) reached Step 7 with `records/artifact-registry.md` and `TREES.md` never placed, and nothing in the flow noticed.
...
3. **No `Werkforce type:` line, but the org chart's apex-seat cell already holds a non-CEO title** (the pattern a real job-type HQ was hand-patched into before this setting existed). I *say what I see* - name the cell and its title - and ask; I never infer and silently write `job`. **Silence default: same as case 2 - `business`.** No answer, or an explicit decline, still becomes a `business` HQ; the only difference from case 2 is that the migration record notes the apex-title evidence I saw and that you were asked and declined, so a later run does not ask again on the same evidence. If you confirm `job`, I add the `Werkforce type: job` line, copy `os/type.md` from `references/types/job.md`, and add `company/role-scorecard.md` from its seed **only if that file does not already exist** - same skip-if-present rule as every other founder-data file in this step. **I do not re-head your existing `company/profile.md` or rewrite its "What we sell"-style headings to the job vocabulary**, even after you confirm `job` - your own words under a shipped heading are founder-authored divergence, untouchable by this migration. Re-heading an existing profile is offered as a separate, one-at-a-time edit you approve later, never bundled into the type confirmation.
...
## Werkforce Starter v0 kernel migration (any 3.x -> 0.1)
...
inactive kernel files. Stamp `os/VERSION` `0.1` only after the backup, file
