...
**When the deliverable is a page**, the acceptance checks usually ask whether it sells. They rarely ask whether it works, and a page that does not work costs a member more than a page that does not convert. So a page review walks three defect lists on top of the brief's own checks, each item something you look at rather than something you feel:

- **Interactivity and states** - no hover state on anything clickable, no pressed feedback, transitions set to zero duration, a missing focus ring (an accessibility requirement, not a style option), a generic spinner where a skeleton belongs, no empty state, no error state, dead links that go to `#`, no current-page indicator in the nav, and anything animating position or size instead of transform and opacity.
- **What got left out** - no legal links, no way back out of a multi-step flow, no custom 404, no validation on a form, no skip-to-content link, and no cookie consent where the jurisdiction requires one. These are the ways a page looks finished and is not.
- **Code quality** - a stack of unlabeled containers where semantic elements belong, hardcoded pixel widths, missing or meaningless alt text, an arbitrary sky-high z-index instead of a stated scale, commented-out dead code, and a missing page title, description, or share preview.

**And one fence, for any page that is already live.** Five things never change without the member being told first, because each is a way to break a working business while appearing to improve it: the URL structure, the nav labels, the form field names, the brand wordmark, and the legal copy. If a revision touched any of them and the handoff did not say so, that is a fault by itself.
...
- **Fix.** Faults remain open, so the close is refused. I hand over the fault list, the board row goes back to In progress, and the Worker fixes exactly what is named - nothing vaguer than that. The next round reads the fix fresh, from the file, not from memory of this round. A refused close also teaches: one lesson line goes to `records/improvements.md` as `- YYYY-MM-DD lesson: {what this fault pattern taught us}`. If the fix waits on something outside the department - a fact only you hold, an external answer - the row can sit at Blocked instead, with `blocked by {what} - recheck {how}` in its Receipt cell until it unblocks.
  ...
  On a page, the fault list is handed back in a fixed order, so the Worker is not left ranking it fresh every time. Most change for least risk first: the typeface, then the palette, then hover and active states, then layout and spacing, then replacing a component outright, then the loading, empty, and error states, and typographic polish last. Faults that block a check jump the queue regardless - the ordering is for the rest.
