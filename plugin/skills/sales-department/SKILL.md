...
**Type lens.** If `os/type.md` exists and names a type other than `business`, I read its vocabulary map and speak this skill's words in that type's words instead - same department mechanics, only the words change. Under `business`, or when `os/type.md` does not exist, nothing here changes.
